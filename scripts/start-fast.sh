...
# 基于LLM的多智能体并行协作系统 - 快速启动脚本

set -e

echo "🚀 快速启动基于LLM的多智能体并行协作系统..."
...
if [ -f "Dockerfile.fast" ]; then
    echo "  使用Dockerfile.fast（无系统依赖安装）"
    docker build -f Dockerfile.fast -t agent-parallel-system:latest .
else
    echo "  使用标准Dockerfile"
...
echo ""
echo "💡 提示："
echo "   这是快速启动版本，使用预构建的Rust镜像"
echo "   构建时间大幅减少，适合快速测试和开发"